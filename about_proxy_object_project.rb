...
class Proxy
  ...
  attr_accessor :messages, :method_calls
  ...
  def method_missing(method_name, *args)
    unless @messages.include?(method_name.to_sym)
      @messages << method_name.to_sym
      @method_calls[method_name.to_sym] = 1
    else
      @method_calls[method_name.to_sym] += 1
    end
  ...
  def number_of_times_called(method_name)
    if @method_calls[method_name.to_sym] == nil
      0
    else
      @method_calls[method_name.to_sym]
    end
  end
